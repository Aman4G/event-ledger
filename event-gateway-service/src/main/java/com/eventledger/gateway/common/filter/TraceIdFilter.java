...
/**
 * Description: TraceIdFilter.java is a servlet filter in the Event Gateway Service that
 * runs on every incoming HTTP request to establish distributed trace context. It extracts
 * the X-Trace-Id header from the request if present, or generates a new UUID trace ID if
